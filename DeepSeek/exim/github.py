...
def init_models():
    return {
        ...
        "language": OllamaLLM(model="llama3.1",num_gpu=1) }

MODELS = init_models()

def stream_json(file, batch_size: int = 1024) -> Generator[Dict, None, None]:
    """Stream JSON content to process large files efficiently"""
    buffer = ""
...
def get_json_text(json_files) -> str:
    ...
    for idx, json_file in enumerate(json_files):
        for content in stream_json(json_file):
            text_chunks.append(process_json_content(content))
...
@st.cache_data(max_entries=1)
def create_embeddings(texts: List[str], batch_size: int = 10) -> InMemoryVectorStore:
...
def get_conversation_chain(vectorstore: InMemoryVectorStore):
    ...
    return ConversationalRetrievalChain.from_llm(
        ...
        retriever=vectorstore.as_retriever(
            search_kwargs={"k": 3}  # Limit search results
        ),
